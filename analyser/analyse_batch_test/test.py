...
def surround_strips(text, regex, size, case):
    """ Printa todos locais onde a match de regex aparece, como também o texto em volta """
    if case:
    ...
    for found in matches:
        ...
        # Se for < 0 vai dar erro, então começo a printar do início
...
def create_detected_img_folder(group_name):
    """
    Create a folder for a group of regex detections
    """
    try:
        Path(os.path.join(FILES_PATH, group_name)).mkdir(parents=True, exist_ok=True)
    except Exception as e:
        print('Failed creating detected img folde', e)
